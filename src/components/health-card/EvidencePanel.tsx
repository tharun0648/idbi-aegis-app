  ...
  return (
    <section className="rounded-xl border border-[#E5E7EB] bg-white p-6">
        ...
        {rows.map(r => {
          ...
          return (
            <li key={r.signal} className="flex items-start gap-3 py-3">
              <span className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[#ECF3F0]">
                <Icon className="h-4 w-4 text-[#1F5E4A]" strokeWidth={1.75} />
              </span>
              <div className="min-w-0 flex-1">
                <p className="text-sm font-medium text-[#111827]">{r.signal}</p>
                <p className="text-xs text-[#6B7280]">{r.value}</p>
              </div>
              <span className="shrink-0 text-sm font-semibold tabular-nums" style={{ color: r.contribution > 0 ? "#1F5E4A" : "#9CA3AF" }}>
      ...
      <div className="mt-2 flex items-baseline justify-between border-t border-[#E5E7EB] pt-4">
        ...
        <span className="text-sm font-semibold tabular-nums text-[#1F5E4A]">
